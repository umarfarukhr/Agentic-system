...
export function ToolConfigurationCard({ tool, onConfigure }: ToolConfigurationCardProps) {
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-lg p-4 flex flex-col transition-opacity shadow-sm ${!tool.enabled ? 'opacity-60' : ''}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center">
          <tool.icon className="w-6 h-6 text-blue-500 mr-3" />
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <button
          ...
          className="flex items-center text-sm font-medium text-slate-700 hover:text-blue-600 bg-slate-100 hover:bg-slate-200 px-3 py-1 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed border border-slate-300"
        >
